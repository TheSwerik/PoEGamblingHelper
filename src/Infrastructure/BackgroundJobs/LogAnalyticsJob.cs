...
public class LogAnalyticsJob(IDateTimeService dateTimeService,
    ...
    : BackgroundJob(configuration, dateTimeService)
{
    ...
    {
        var yesterday = dateTimeService.UtcToday().AddDays(-1);

        var views = await viewRepository.CountViewsAsync(yesterday);
